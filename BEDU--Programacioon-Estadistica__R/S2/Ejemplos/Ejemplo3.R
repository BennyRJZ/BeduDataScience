...
conf <- read.csv("/Users/benny/Documents/BeduDataScience/BEDU--Programacioon-Estadistica__R/S2/Ejemplos/Datasets/st19ncov-confirmados.csv")
mu <- read.csv("/Users/benny/Documents/BeduDataScience/BEDU--Programacioon-Estadistica__R/S2/Ejemplos/Datasets/st19ncov-muertes.csv")
...
Sconf <- conf[-1,] #quitamos la primer fila de todas las columnas
Smu <- mu[-1,] #quitamos la primer fila de todas las columnas
...
#         rename  nombre del datafrme,  datos a renombrar
Sconf <- rename(Sconf, Country = Country.Region, Infectados = Value)

str(Sconf)
names(Sconf) #columnas del dataset.
...
# mutate, dataframe, change of the variable, asdate(df,format)
